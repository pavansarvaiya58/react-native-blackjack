...
class App extends Component {
	...
	_handleAppStateChange = (nextAppState) => {
		...
		// 	if (Platform.OS === "android") {
		// 		AudioPlayer.loadAudio("audio/sound.mp3", (message) => {
		// 			if (message) {
		// 				AudioPlayer.play();
		// 			}
		// 		});
	...
	render() {
		const { totalBet, amount, playerHand, dealerHand, gameover, gameMessage } =
			this.state;
		return (
			<ImageBackground source={boardBg} style={styles.container}>
				...
				<View style={styles.bottom}>
					...
					/>
					<View style={styles.center}>
						<FloatingText text={`Available $ ${amount}`} available={true} />
					</View>
